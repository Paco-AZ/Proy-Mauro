...
import servicios.reportes.ReportePDF;
...
public class Consultas extends javax.swing.JFrame
{
    ...
    private void initComponents()
    {
        ...
        jPanel4.add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(470, 640, -1, -1));
...
//        cbxFrecuencia.setVisible(false);
    ...
    {//GEN-HEADEREND:event_btnBuscarActionPerformed
        // TODO add your handling code here:
        String cve = CveRegistro.getText();
        miControlador.cargarHistorial(cve, labelNom, labelAp, labelSegAp, listCitas);
    ...
    {//GEN-HEADEREND:event_listCitasValueChanged
        ...
        {
            Consulta seleccion = listCitas.getSelectedValue();
            ...
            labelP.setText(seleccion.getPadecimiento());
            labelAntece.setText(seleccion.getAntecedentes());
            labelM.setText(seleccion.getMedicamento());
            labelPT.setText(seleccion.getPlanTratamiento());
        }
    ...
    {//GEN-HEADEREND:event_btnEliminaActionPerformed
        ...
        if (selectedRow == -1)
        {
            Mensajes.error(this, "Seleccione una fila en la tabla para eliminar.");
            return;
        }

        try
        {
            // 2. Extraer el dato de la celda (Columna 0 = Clave/ID)
            int cve = Integer.parseInt(tabla.getValueAt(selectedRow, 0).toString());

            // 3. Delegar TODA la lógica pesada al Controlador
            boolean borradoExitoso = miControlador.eliminarRegistro(cve, Principal.tipo, this);

            // 4. Si el Controlador confirma que se borró de MySQL, refrescamos la tabla
            if (borradoExitoso)
            {
                filtra();
            }
        ...
        {
            Mensajes.error(this, "Error de formato: La clave seleccionada no es un número válido.");
        } catch (Exception e)
        {
            Mensajes.error(this, "Error inesperado al intentar leer la fila seleccionada.");
        }
    ...
    {//GEN-HEADEREND:event_CveRegistro1KeyTyped
        ...
        if (!CveRegistro1.getText().trim().isEmpty())
        {
    ...
    {//GEN-HEADEREND:event_jcbxMuestraTActionPerformed
        ...
        {
            // 1. LÓGICA PURAMENTE VISUAL (Se queda en la Vista)
            ...
            // 2. LÓGICA DE NEGOCIO (El Controlador orquesta, la Vista solo le pasa la tabla)
            miControlador.cargarTodosLosDatos(Principal.tipo, tabla);

        } else
        {
            // 3. LIMPIEZA VISUAL (Como es solo limpiar la pantalla, está bien dejarlo aquí)
            ((DefaultTableModel) tabla.getModel()).setRowCount(0);
        }
    ...
    {//GEN-HEADEREND:event_jButton1ActionPerformed
        int indexSexo = cbxSexo.getSelectedIndex();
        int indexEstatus = cbxEstatus.getSelectedIndex(); // Si no aplica (ej. es alumno), envía 0
        int indexFrecuencia = cbxFrecuencia.getSelectedIndex();
        int indexViveCon = cbxViveCon.getSelectedIndex(); // Si no aplica (ej. es personal), envía 0
        int indexCarreras = cbxCarreras.getSelectedIndex(); // Si no aplica, envía 0

        boolean desnutricion = jcbxDesnutricion.isSelected();
        boolean sobrepeso = jcbxSobrepeso.isSelected();
        boolean alergias = jcbxAlergias.isSelected();
        boolean obesidad = jcbxObesidad.isSelected();
        boolean diabetes = jcbxDiabetes.isSelected();
        boolean otras = jcbxOtras.isSelected();
        try
        {
            // 1. Recolectar datos actuales (puedes leer los JComboBox directamente aquí)
            // 2. Pedir la lista al controlador
            List<Object[]> listaDatos = miControlador.obtenerListaFiltrada(
                    indexSexo, indexEstatus, indexFrecuencia,
                    indexViveCon, indexCarreras, desnutricion, sobrepeso,
                    alergias, obesidad, diabetes, otras,Principal.tipo
            );            
            // 3. Calcular el predominante
            String predominante = miControlador.obtenerPredominante(listaDatos, Principal.tipo);

            // 4. Generar el PDF
            ReportePDF reporte = new ReportePDF();
            reporte.generarConsultaGeneral("Reporte de Salud", predominante, listaDatos, "Reporte.pdf");

            JOptionPane.showMessageDialog(this, "PDF generado con éxito.");

        } catch (Exception e)
        {
            Mensajes.error(this, "Error al generar PDF: " + e.getMessage());
        }
    ...
    public void filtra()
    {
        ...
        // 2. Le mandamos todo el paquete al Controlador
        // Nota: El Controlador también debe recibir la tabla que va a actualizar
        miControlador.aplicarFiltrosYActualizarTabla(
                indexSexo, indexEstatus, indexFrecuencia, indexViveCon, indexCarreras,
                desnutricion, sobrepeso, alergias, obesidad, diabetes, otras,
                Principal.tipo, // Pasamos el tipo (1=Alumno, 2=Personal)
                tabla // Pasamos el JTable
        );

        // 3. Lógica visual: Si todo está vacío, habilitar "Mostrar Todos"
        boolean todoVacio = (indexSexo == 0 && indexEstatus == 0 && indexFrecuencia == 0
                && indexViveCon == 0 && indexCarreras == 0
                && !desnutricion && !sobrepeso && !alergias && !obesidad && !diabetes && !otras);

        CtrlInterfaz.habilitaChB(1, todoVacio, jcbxMuestraT);
    }
...
//}
    ...
    private javax.swing.JList<Consulta> listCitas;
